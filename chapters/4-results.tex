...
\matteo{Bug deduplication may require a section  on its own; it is an interesting thing to describe! You should probably put it at the end of the previous chapter (i.e. methodology) and explain in detail how we performed it. Do not describe the scripts you used, but talk about things like callstack deduplication (k=3) for ASan and valgrind (and spontaneous crashes IIRC?), while for UBSan we looked at both the address and the type of the bug.}
...
\matteo{Refer to table as "Table~\ref{xxx}", not as "above/below table". Do not try to force latex to place things the way you want, but let it do its things and use labels to refer to things. When you create a table you can also suggest latex where to put it using "[h]" or "[h!]", but in the end let it decide. This is also true for newlined and newpages, btw.}
...
\matteo{Some interesting things we can say to expand this section: 1) importance of having sanitizers on OSS-Fuzz (gpac does not have any) 2) say something about the public commit they made about our report (I am just talking about the text, I'll link it in a comment below) like the functional concerns and the good practices 3) maybe describe one of the bugs (read briefly the commit to see if you can spot easily the root cause of one of the bugs and describe it briefly here; no need to copy paste code or add images)}

%https://github.com/gpac/gpac/pull/2965
...
\matteo{Makes sense, discuss this "perceived priority" in the discussion section.}
...
\matteo{I would avoid writing this. We contacted all the authors according to how their security policies (that you checked); some of them were even contacted through mail (e.g. gpac). Writing that this is the only report we made through the security sections seems like we either reported the other ones in a non-secure way or that the other bugs were not security relevant.}
...
\paragraph{Perceived priority} \matteo{Say how developers seem to care more about certain reports (e.g. ASan) and care less about other (e.g. UBSan). Maybe link some vulnerabilities from the CVE database where you show that undefined behaviors can still trigger something like remote command execution or privilege escalation (go in the db and search for something like integer overflows).}
...
\matteo{We can conclude by saying that we encourage the use of multiple sanitizers (as many as possible), and of multiple fuzzing engines to not miss obvious bugs. Then, we suggest maintainers of such frameworks to periodically check fuzzing queues as well as their settings to constantly look for issues in their testing pipeline.}
...
\matteo{We can say that, when testing older versions of the program, it is also necessary to check whether those bugs are still present in the latest versions of the program under testing. This has to hold for automated frameworks like fuzzing but has to hold for short-timed events like sbft. In this last case, those responsible for the challenge failed to do so and found many valuable bugs that were open to anyone but were not reported.}