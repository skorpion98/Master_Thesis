...
\textbf{Testcase.}\ \ \ Data taken from an external source and provided as input to the tested program to observe its behavior, usually in the form of bytes arrays. Each testcase is defined by a set of metadata like ID, description, and expected result. The first fuzzing session takes a set of inputs defined and provided by the developer itself, while future fuzzing sessions will also rely on previously discovered "interesting inputs". 
