...
\matteo{This is better now; I would suggest you stress a bit more how fuzzing works, right after the first paragraph. Maybe you can use a picture. Keep in mind that whoever is going to read this has most likely never heard about fuzzing, so it is worth explaining it carefully.}
...
\matteo{The classification based on "program's structure awareness" is far more important than this. I suggest you discuss deeply that one, and only briefly discuss this one later. Also, protocol fuzzing and application fuzzing are much less developed than "file format fuzzing".}
...
\matteo{This section is not "Pros and Cons", this section is only about "Cons". You are basically making a half-page long discussion on why fuzzing sucks; moreover, most (if not all) these arguments are either false or exaggerated. Given that your thesis revolves around how to make fuzzing frameworks more precise, I suggest you remove this unless you want to give the impression that fuzzing sucks and thus your work is useless. Anyway, if you want to make such strong statements, you should cite some source (a reliable one, like a decent paper)}

While fuzzing is a very effective and popular technique, it's important to mention that there is a trade-off between results obtained, time spent testing and resources available.
...
\matteo{Add references when you cite numbers. 3.4x seems also a bit too high; it should be 2x, but I might be wrong}
\ziosaba{So it says in the first few lines of "Introduction" from the official paper, the wiki states an an average of 2, dunno which is more truthful}
...
\matteo{Again, add some source for these numbers}
...
\ziosaba{Stated in the abstract from the official paper}

\subsection{UBSan}

\matteo{You describe ASan and MSan with a decent level of details, but you only write a couple of things about UBSan. Try to say something more, study something about how it works and write it. (e.g., what is an undefined behavior? What are some example of it? (please don't add a page long table with a list of undefined behaviors) How dangerous are they? ...)}

The \textit{Undefined Behavior Sanitizer} \cite{ubsan_docs} is a fast undefined-behavior detector for C/C++, that helps developers to find and fix undefined-behavior (UB) bugs. Common operations that may lead to undefined behaviors are: array subscription out of bounds, overflows/underflows originating from mathematical or logical operations, dereferenced, misaligned or null pointers and conversion between data types. This is because \matteo{<-- broken sentence}
...
\newpage
\matteo{You are citing "the tests" you do later, which you have not introduced yet, so this will cause lots of confusion when someone reads it. Moreover, valgrind does both the job of MSan and ASan (even if ASan stuff is only performed on the heap); maybe this could become a new subsection where you talk about binary-level sanitizers?}
